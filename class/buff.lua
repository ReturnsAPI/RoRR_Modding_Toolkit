...
gm.pre_script_hook(gm.constants.step_actor, function(self, other, result, args)
    ...
    if callbacks["onPreStep"] then
        for _, buff_id in ipairs(iteration_lookup[actor.id]) do
            local c_table = callbacks["onPreStep"][buff_id]
            if c_table then
...
gm.post_script_hook(gm.constants.step_actor, function(self, other, result, args)
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks["onPostStep"][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
gm.pre_script_hook(gm.constants.draw_actor, function(self, other, result, args)
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks["onPreDraw"][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
gm.post_script_hook(gm.constants.draw_actor, function(self, other, result, args)
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks["onPostDraw"][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
gm.post_script_hook(gm.constants.recalculate_stats, function(self, other, result, args)
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks["onStatRecalc"][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
gm.post_script_hook(gm.constants.skill_activate, function(self, other, result, args)
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks[callback][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
function buff_onPostStatRecalc(actor)
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks["onPostStatRecalc"][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
Callback_Raw.add("onAttackCreate", "RMT-Buff.onAttackCreate", function(self, other, result, args)
    ...
    if callbacks["onAttackCreate"] then
        for _, buff_id in ipairs(iteration_lookup[actor.id]) do
            local c_table = callbacks["onAttackCreate"][buff_id]
            if c_table then
                local stack = actor:buff_stack_count(buff_id)
                if stack > 0 then
                    for _, fn in ipairs(c_table) do
    ...
    if callbacks["onAttackCreateProc"] then
        for _, buff_id in ipairs(iteration_lookup[actor.id]) do
            local c_table = callbacks["onAttackCreateProc"][buff_id]
            if c_table then
                local stack = actor:buff_stack_count(buff_id)
                if stack > 0 then
                    for _, fn in ipairs(c_table) do
...
Callback_Raw.add("onAttackHit", "RMT-Buff.onAttackHit", function(self, other, result, args)
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks["onAttackHit"][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
Callback_Raw.add("onAttackHandleEnd", "RMT-Buff.onAttackHandleEnd", function(self, other, result, args)
    local attack_info = Attack_Info.wrap(args[2].value)
    local actor = attack_info.parent
...
Callback_Raw.add("onHitProc", "RMT-Buff.onHitProc", function(self, other, result, args)     -- Runs before onAttackHit
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks["onHitProc"][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
Callback_Raw.add("onKillProc", "RMT-Buff.onKillProc", function(self, other, result, args)
    ...
    for _, buff_id in ipairs(iteration_lookup[actor.id]) do
        local c_table = callbacks["onKillProc"][buff_id]
        if c_table then
            local stack = actor:buff_stack_count(buff_id)
            if stack > 0 then
                for _, fn in ipairs(c_table) do
...
Callback_Raw.add("onDamagedProc", "RMT-Buff.onDamagedProc", function(self, other, result, args)
...
Callback_Raw.add("onDamageBlocked", "RMT-Buff.onDamageBlocked", function(self, other, result, args)
...
Callback_Raw.add("onInteractableActivate", "RMT-Buff.onInteractableActivate", function(self, other, result, args)
...
Callback_Raw.add("onPickupCollected", "RMT-Buff.onPickupCollected", function(self, other, result, args)
...
Callback_Raw.add("onEquipmentUse", "RMT-Buff.onEquipmentUse", function(self, other, result, args)
...
Callback_Raw.add("onStageStart", "RMT-Buff.onStageStart", function(self, other, result, args)
